...
#define POP_BARALHO_CMD			"=popbaralho"

BAR_tpBaralho Baralho = NULL;
CAR_tpCarta cCarta = NULL;
TST_tpCondRet TST_EfetuarComando(char * ComandoTeste)
{
	...
	/* Testar popBaralho */
	else if (strcmp(ComandoTeste, POP_BARALHO_CMD) == 0)
	{
		numLidos = LER_LerParametros("i", &CondRetEsperada);
		...
		CondRetRecebida = BAR_popBaralho(Baralho, &cCarta);

		return TST_CompararInt(CondRetEsperada, CondRetRecebida, "Erro ao tirar uma carta do baralho.");
	}
	/* fim ativa: Testar criarBaralho */
