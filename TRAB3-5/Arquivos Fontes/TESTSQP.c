...
static const char RETORNA_PILHA_SQP_CMD [ ] = "=retornapilha";
static const char PUSH_SQP_CMD			[ ] = "=pushsqp";
static const char POP_SQP_CMD			[ ] = "=popsqp";
...
TST_tpCondRet TST_EfetuarComando(char * ComandoTeste)
{
	...
	else if (strcmp(ComandoTeste, REMOVER_SQP_CMD) == 0)
	{
		...
		CondRetRecebida = SQP_removeDaSequencia(sqPrincipal, cCarta9, &pPilha2);
	...
	else if (strcmp(ComandoTeste, ADICIONAR_SQP_CMD) == 0)
	{
		...
		return TST_CompararInt(CondRetEsperada, CondRetRecebida, "Erro ao adicionar a SQP!");

	} /* fim ativa: Testar SQP adicionaNaSequencia */


	/*Testar SQP retornaPilha*/
	else if (strcmp(ComandoTeste, RETORNA_PILHA_SQP_CMD) == 0)
	{
		...
		CondRetRecebida = SQP_retornaPilha(sqPrincipal, &pSQP);

		return TST_CompararInt(CondRetEsperada, CondRetRecebida, "Erro ao retorna pilha da SQP!");

	} /* fim ativa: Testar SQP retornaPilha */

	/*Testar SQP pushSQP*/
	else if (strcmp(ComandoTeste, PUSH_SQP_CMD) == 0)
	{
		...
		CondRetRecebida = SQP_pushSQP(sqPrincipal, cPopSQP);

		return TST_CompararInt(CondRetEsperada, CondRetRecebida, "Erro ao PUSH carta para SQP!");

	} /* fim ativa: Testar SQP pushSQP */

	/*Testar SQP popSQP*/
	else if (strcmp(ComandoTeste, POP_SQP_CMD) == 0)
	{
		...
		CondRetRecebida = SQP_popSQP(sqPrincipal, &cPopSQP);

		return TST_CompararInt(CondRetEsperada, CondRetRecebida, "Erro ao POP da SQP para cPopSQP!");

	} /* fim ativa: Testar SQP popSQP */

	/*Testar SQP verficaSequenciaCompleta*/
	else if (strcmp(ComandoTeste, VERIFICA_SQP_CMD) == 0)
	{
		...
		CondRetRecebida = SQP_verificaSequenciaCompleta(sqPrincipal);

		return TST_CompararInt(CondRetEsperada, CondRetRecebida, "Erro ao verificar ao SQP");

	} /* fim ativa: Testar SQP pverficaSequenciaCompleta*/
